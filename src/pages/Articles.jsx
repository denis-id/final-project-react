                ...
                <div>
                  <div className="flex items-center gap-4 text-gray-500 text-sm mb-4">
                    ...
                    <div className="flex items-center">
                      ...
                      {translations[language]?.articles?.find((item) => item.id === featuredArticle.id)?.readTime}
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold mb-4">
                    {translations[language]?.articles?.find((item) => item.id === featuredArticle.id)?.title}
                  </h3>
                  <p className="text-gray-600 mb-4">
                    {translations[language]?.articles?.find((item) => item.id === featuredArticle.id)?.excerpt}
                  </p>
            ...
            {regularArticles.slice(0, 6).map((article) => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  <img
                    src={article.image}
                    alt={translations[language]?.articles?.find((item) => item.id === articles.id)?.title}
                ...
                <div className="p-6">
                  <div className="flex items-center gap-4 text-gray-500 text-sm mb-4">
                    ...
                    <div className="flex items-center">
                      <Clock className="w-4 h-4 mr-2" />
                      {translations[language]?.articles?.find((item) => item.id === article.id)?.readTime}
                    </div>
                  </div>
                  <h3 className="text-xl font-bold mb-3 group-hover:text-gray-600 transition-colors">
                    {translations[language]?.articles?.find((item) => item.id === article.id)?.title}
                  </h3>
                  <p className="text-gray-600 mb-4 line-clamp-2">
                    {translations[language]?.articles?.find((item) => item.id === article.id)?.excerpt}
                  </p>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500 text-sm">{translations[language]?.writerArticle}: {article.author}</span>
                    <button
